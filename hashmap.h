...
#define hashmap_init(entries)                                                                      \
  do {                                                                                             \
    if (entries)                                                                                   \
      break;                                                                                       \
    HashMapHdr* hdr = xmalloc(HASHMAP_MIN_CAP * sizeof *(entries) + sizeof *hdr);                  \
    entries = (void*)(hdr + 1);                                                                    \
...
#define arena_hashmap_init(arena, entries)                                                         \
  do {                                                                                             \
    if (entries)                                                                                   \
      break;                                                                                       \
    HashMapHdr* hdr =                                                                              \
        (HashMapHdr*)arena_alloc((arena), HASHMAP_MIN_CAP * sizeof *(entries) + sizeof *hdr);      \
    entries = (void*)(hdr + 1);                                                                    \
    hdr->cap = HASHMAP_MIN_CAP;                                                                    \
    hdr->size = 0;                                                                                 \
    hdr->deleted_count = 0;                                                                        \
...
#define hashmap_resize(entries, new_cap)                                                           \
  do {                                                                                             \
    ...
    if (real_new_cap == hdr->cap)                                                                  \
      break;                                                                                       \
                                                                                                   \
    HashMapHdr* new_hdr = xmalloc(real_new_cap * sizeof *(entries) + sizeof *new_hdr);             \
...
#define arena_hashmap_resize(arena, entries, new_cap)                                              \
  do {                                                                                             \
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
    size_t real_new_cap = MAX((new_cap), HASHMAP_MIN_CAP);                                         \
                                                                                                   \
    if (real_new_cap == hdr->cap)                                                                  \
      break;                                                                                       \
                                                                                                   \
    HashMapHdr* new_hdr =                                                                          \
        (HashMapHdr*)arena_alloc((arena), real_new_cap * sizeof *(entries) + sizeof *new_hdr);     \
    new_hdr->cap = real_new_cap;                                                                   \
    new_hdr->size = hdr->size;                                                                     \
    new_hdr->destructor = hdr->destructor;                                                         \
    new_hdr->deleted_count = 0;                                                                    \
...
#define hashmap_ensure_cap(entries)                                                                \
  do {                                                                                             \
    ...
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
                                                                                                   \
    if ((((float)(hdr->size + hdr->deleted_count)) / hdr->cap) < HASHMAP_LOAD_FACTOR)              \
      break;                                                                                       \
                                                                                                   \
...
#define arena_hashmap_ensure_cap(arena, entries)                                                   \
  do {                                                                                             \
    arena_hashmap_init((arena), (entries));                                                        \
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
                                                                                                   \
    if ((((float)(hdr->size + hdr->deleted_count)) / hdr->cap) < HASHMAP_LOAD_FACTOR)              \
      break;                                                                                       \
                                                                                                   \
    size_t new_cap = hdr->cap * 2;                                                                 \
    arena_hashmap_resize((arena), (entries), new_cap);                                             \
  } while (0);
...
#define arena_hashmap_set(arena, entries, k, v)                                                    \
  do {                                                                                             \
    arena_hashmap_ensure_cap((arena), (entries));                                                  \
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
                                                                                                   \
    size_t idx = hashmap_hash_key((k)) % hdr->cap;                                                 \
    size_t checked = 0;                                                                            \
      ...
      if (entry->state == HASHMAP_ENTRY_EMPTY) {                                                   \
        if (first_deleted_idx != (size_t)-1) {                                                     \
          ...
          hdr->deleted_count--;                                                                    \
                                                                                                   ...
                                                                                                   \
        entry = &(entries)[idx];                                                                   \
        entry->key = arena_strdup((arena), (k));                                                   \
        entry->state = HASHMAP_ENTRY_OCCUPIED;                                                     \
        entry->value = (v);                                                                        \
        hdr->size++;                                                                               \
        break;                                                                                     \
...
static inline size_t hashmap_get_idx(void* entries, const char* key) {
  ...
  while (checked < hdr->cap) {
    ...
    HASHMAP_ENTRY_STATE state = *(HASHMAP_ENTRY_STATE*)(entry + sizeof(const char*));
...
#define hashmap_get_ref(entries, k, out) ((*(out) = hashmap_get_ref_impl(entries, k)) != NULL)

static inline bool hashmap_get(void* entries, const char* key, void* out) {
  HashMapHdr* hdr = HASHMAP_HDR(entries);
...
#define arena_hashmap_delete(entries, k)                                                           \
  do {                                                                                             \
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
    size_t idx = hashmap_get_idx(entries, (k));                                                    \
                                                                                                   \
...
#define hashmap_destroy(entries)                                                                   \
  do {                                                                                             \
    if (!(entries))                                                                                \
      break;                                                                                       \
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
...
// "Free" for arena-backed hashmaps: runs the value destructor (if set) and
// clears the table. Backing storage and keys (arena memory) are not freed.
#define arena_hashmap_clear(entries)                                                               \
  do {                                                                                             \
    if (!(entries))                                                                                \
      break;                                                                                       \
    HashMapHdr* hdr = HASHMAP_HDR(entries);                                                        \
    ...
    for (size_t i = 0; i < hdr->cap; ++i) {                                                        \
      typeof(entries) entry = &(entries)[i];                                                       \
      if (entry->state == HASHMAP_ENTRY_OCCUPIED && hdr->destructor)                               \
        hdr->destructor(&entry->value);                                                            \
      entry->key = NULL;                                                                           \
      entry->state = HASHMAP_ENTRY_EMPTY;                                                          \
    }                                                                                              \

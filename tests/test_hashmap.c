...
void test_insertion() {
  ...
  TEST_ASSERT(string_items[idx].key != NULL, "Entry at expected slot must not be NULL");
  TEST_ASSERT(strcmp(string_items[idx].key, "key1") == 0, "Key must match 'key1'");
  TEST_ASSERT(strcmp(string_items[idx].value, "world") == 0, "Value must match 'world'");
  TEST_ASSERT(hdr->size == 1, "Map size must be 1 after one insertion, got %zu", hdr->size);
}
...
void test_overwrite() {
  ...
  TEST_ASSERT(hdr->size == 1, "Map size must still be 1 after overwrite, got %zu", hdr->size);
}
...
void test_getting_elements() {
  ...
  TEST_ASSERT(hashmap_get_idx(string_items, key) == hashmap_hash_key(key) % hdr->cap,
  ...
  TEST_ASSERT(strcmp(*ref, "Hello") == 0, "ref value must be 'Hello', got '%s'", *ref);
  ...
  *ref = "World!";
  ...
  TEST_ASSERT(strcmp(copy, "World!") == 0, "Copied value must reflect mutation via ref, got '%s'",
              copy);
...
void test_deleting_elements() {
  ...
  HashMapHdr* hdr = HASHMAP_HDR(string_items);
  TEST_ASSERT(hdr->size == 2, "Size must be 2 before deletion, got %zu", hdr->size);

  hashmap_delete(string_items, "key1");

  TEST_ASSERT(hdr->size == 1, "Size must be 1 after deletion, got %zu", hdr->size);
  TEST_ASSERT(hdr->deleted_count == 1, "deleted_count must be 1, got %zu", hdr->deleted_count);

  char** value = NULL;
  TEST_ASSERT(!hashmap_get_ref(string_items, "key1", &value), "Deleted key must not be found");
  TEST_ASSERT(hashmap_get_ref(string_items, "key2", &value), "Non-deleted key must still be found");

  hashmap_delete(string_items, "nonexistent");
  TEST_ASSERT(hdr->size == 1, "Size must remain 1 after deleting nonexistent key, got %zu",
              hdr->size);
}
...
void test_destructor() {
  ...
  hashmap_delete(string_items, "key1");
  TEST_ASSERT(destructor_called == 1, "Destructor must be called once on delete, called %d times",
              destructor_called);

  char* value2 = strdup("World");
  hashmap_set(string_items, "key2", value2);
  hashmap_set(string_items, "key2", strdup("Overwritten"));
  TEST_ASSERT(destructor_called == 2, "Destructor must be called on overwrite too, called %d times",
              destructor_called);
}
...
void test_resize() {
  ...
  HashMapHdr* hdr = HASHMAP_HDR(string_items);
  TEST_ASSERT(hdr->size == 8, "Size must be 8 after 8 insertions, got %zu", hdr->size);
  TEST_ASSERT(hdr->cap > 4, "Capacity must have grown beyond 4, got %zu", hdr->cap);
...
void test_arena_variant() {
  TEST_SUITE("Arena variant");

  Arena arena = arena_create(1024);

  StringItem* items = NULL;
  arena_hashmap_set(&arena, items, "k1", "v1");
  arena_hashmap_set(&arena, items, "k2", "v2");
  arena_hashmap_set(&arena, items, "k3", "v3");

  char** ref = NULL;
  TEST_ASSERT(hashmap_get_ref(items, "k2", &ref), "arena_hashmap_set keys must be retrievable");
  TEST_ASSERT(strcmp(*ref, "v2") == 0, "value for k2 must be v2");

  arena_hashmap_set(&arena, items, "k2", "v2b");
  TEST_ASSERT(hashmap_get_ref(items, "k2", &ref), "k2 must still exist");
  TEST_ASSERT(strcmp(*ref, "v2b") == 0, "overwrite must update value");

  arena_hashmap_delete(items, "k1");
  TEST_ASSERT(!hashmap_get_ref(items, "k1", &ref), "deleted key must not be found");

  // Force growth
  for (int i = 0; i < 64; i++) {
    Dstr k = arena_dstr_fmt(&arena, "key_%d", i);
    arena_hashmap_set(&arena, items, k, "x");
  }
  HashMapHdr* hdr = HASHMAP_HDR(items);
  TEST_ASSERT(hdr->cap > HASHMAP_MIN_CAP, "arena hashmap must grow");

  arena_destroy(&arena);
}

void test_free_and_clear() {
  TEST_SUITE("Free / clear");

  StringItem* items = NULL;
  hashmap_set(items, "k1", strdup("v1"));
  hashmap_set(items, "k2", strdup("v2"));
  hashmap_set_destructor(items, free_string);

  destructor_called = 0;
  hashmap_clear(items);
  TEST_ASSERT(destructor_called == 2, "hashmap_clear must call destructor for all values");

  // Reuse after clear
  hashmap_set(items, "k3", strdup("v3"));
  destructor_called = 0;
  hashmap_destroy(items);
  TEST_ASSERT(items == NULL, "hashmap_free must set items to NULL");
  TEST_ASSERT(destructor_called == 1, "hashmap_free must call destructor for remaining values");
}

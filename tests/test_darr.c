...
void test_init() {
  ...
  TEST_ASSERT(darr_len(arr) == 0, "Length must be 0 after init, got %zu",
              darr_len(arr));
  TEST_ASSERT(darr_cap(arr) == MIN_DYNAMIC_ARRAY_SIZE,
              "Cap must be %d after init, got %zu", MIN_DYNAMIC_ARRAY_SIZE,
              darr_cap(arr));
...
void test_resize() {
  ...
  TEST_ASSERT(darr_len(arr) == 16,
              "Length must be 16 after 16 appends, got %zu", darr_len(arr));
...
void test_pop() {
  ...
  darr_pop(arr);
  TEST_ASSERT(darr_len(arr) == 2, "Length must be 2 after pop, got %zu",
              darr_len(arr));
  ...
  darr_pop(arr);
  darr_pop(arr);
  TEST_ASSERT(darr_len(arr) == 0, "Length must be 0 after popping all, got %zu",
              darr_len(arr));

  // pop on empty must not crash
  darr_pop(arr);
  TEST_ASSERT(darr_len(arr) == 0, "Pop on empty must be a no-op, got %zu",
              darr_len(arr));
}
...
void test_set() {
  ...
  TEST_ASSERT(darr_len(arr) == 3, "Length must remain 3, got %zu",
              darr_len(arr));

  // set at size — acts like append
  darr_set(arr, 3, 99);
  TEST_ASSERT(darr_len(arr) == 4, "Length must be 4 after set at end, got %zu",
              darr_len(arr));
...
void test_insert_at() {
  ...
  TEST_ASSERT(darr_len(arr) == 4, "Length must be 4 after insert, got %zu",
              darr_len(arr));
  ...
  TEST_ASSERT(arr[0] == 99, "arr[0] must be 99 after insert at 0, got %d",
              arr[0]);
  TEST_ASSERT(darr_len(arr) == 5, "Length must be 5, got %zu", darr_len(arr));

  // insert at end
  darr_insert_at(arr, darr_len(arr), 77);
  TEST_ASSERT(arr[darr_len(arr) - 1] == 77,
              "Last element must be 77 after insert at end, got %d",
              arr[darr_len(arr) - 1]);
}
...
void test_remove_at() {
  ...
  TEST_ASSERT(darr_len(arr) == 3, "Length must be 3 after remove, got %zu",
              darr_len(arr));
  TEST_ASSERT(arr[0] == 10, "arr[0] must be 10, got %d", arr[0]);
  ...
  TEST_ASSERT(arr[0] == 30, "arr[0] must be 30 after removing first, got %d",
              arr[0]);
...
void test_append_range() {
  ...
  TEST_ASSERT(darr_len(arr) == 5, "Length must be 5, got %zu", darr_len(arr));
  for (int i = 0; i < 5; i++)
    TEST_ASSERT(arr[i] == i + 1, "arr[%d] must be %d, got %d", i, i + 1,
                arr[i]);
  ...
  TEST_ASSERT(darr_len(arr2) == 3,
              "Length must be 3 after append_range into NULL, got %zu",
              darr_len(arr2));
...
void test_reserve() {
  ...
  TEST_ASSERT(darr_cap(arr) == 64, "Cap must be 64 after reserve, got %zu",
              darr_cap(arr));
  TEST_ASSERT(darr_len(arr) == 0, "Length must still be 0, got %zu",
              darr_len(arr));
  ...
  TEST_ASSERT(darr_cap(arr) == 64, "Cap must still be 64, got %zu",
              darr_cap(arr));
}
...
void test_clone() {
  ...
  TEST_ASSERT(darr_len(dst) == darr_len(src),
              "Cloned length must match source, got %zu vs %zu", darr_len(dst),
              darr_len(src));

  for (size_t i = 0; i < darr_len(src); i++)
    TEST_ASSERT(dst[i] == src[i], "dst[%zu] must equal src[%zu]: %d vs %d", i,
                i, dst[i], src[i]);

  // mutating dst must not affect src
  dst[0] = 99;
  TEST_ASSERT(src[0] == 1, "src[0] must be unaffected by dst mutation, got %d",
              src[0]);
}
...
void test_destructor() {
  ...
  TEST_ASSERT(destructor_called == 1,
              "Destructor must be called once on pop, called %d times",
              destructor_called);

  darr_remove_at(arr, 0);
  TEST_ASSERT(destructor_called == 2,
              "Destructor must be called on remove_at, called %d times",
              destructor_called);
...
void test_clear() {
  ...
  TEST_ASSERT(darr_len(arr) == 0, "Length must be 0 after clear, got %zu",
              darr_len(arr));
  TEST_ASSERT(darr_cap(arr) == cap_before,
              "Cap must be unchanged after clear, got %zu vs %zu",
              darr_cap(arr), cap_before);

  // must be usable after clear
  darr_append(arr, 42);
  TEST_ASSERT(darr_len(arr) == 1, "Must be usable after clear, length got %zu",
              darr_len(arr));
  TEST_ASSERT(arr[0] == 42, "arr[0] must be 42 after re-append, got %d",
              arr[0]);
}
